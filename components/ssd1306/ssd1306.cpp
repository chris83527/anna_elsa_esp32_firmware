...
int SSD1306::get_width() const { return this->_width; }

int SSD1306::get_height() const { return this->_height; }

int SSD1306::get_pages() const { return this->_pages; }
...
void SSD1306::set_buffer(const uint8_t* buffer)
{
    int index = 0;
    for (int page = 0; page < this->_pages; page++)
    {
...
void SSD1306::get_buffer(uint8_t* buffer) const
{
    int index = 0;
    for (int page = 0; page < this->_pages; page++)
    {
...
void SSD1306::display_text(int page, const std::string& text, bool invert)
{
    ...
    {
        memcpy(image, font8x8_basic_tr[static_cast<uint8_t>(newText[i])], 8);
        if (invert)
...
void SSD1306::display_text_x3(int page, const std::string& text, bool invert)
{
    ...
    for (uint8_t nn = 0; nn < text.length(); nn++)
    {
        uint8_t const* const in_columns =
            font8x8_basic_tr[static_cast<uint8_t>(newText[nn])];
             ...
             yy++)
        {
            ...
            if (invert)
                SSD1306::invert(image, 24);
            if (this->_flip)
                SSD1306::flip(image, 24);
...
void SSD1306::scroll_text(const std::string& text, bool invert)
{
    ...
    while (true)
    {
        ...
        i2c_display_image(dstIndex, 0, this->_page[dstIndex]._segs,
                          sizeof(this->_page[dstIndex]._segs));
...
void SSD1306::bitmaps(int xpos, int ypos, const uint8_t* bitmap, int tmpWidth,
...
void SSD1306::dump_page(int page, int seg) const
{
    ESP_LOGD(TAG, "this->_page[%d]._segs[%d]=%02x", page, seg,
             this->_page[page]._segs[seg]);
}
