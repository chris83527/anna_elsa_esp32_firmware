...
int SSD1306::get_width() { return this->_width; }

int SSD1306::get_height() { return this->_height; }

int SSD1306::get_pages() { return this->_pages; }
...
void SSD1306::display_text_x3(int page, const std::string text, bool invert) {
  ...
  for (uint8_t nn = 0; nn < text.length(); nn++) {
    ...
    uint8_t const *const in_columns =
        font8x8_basic_tr[(uint8_t)newText.data()[nn]];
...
void SSD1306::bitmaps(int xpos, int ypos, uint8_t *bitmap, int width,
