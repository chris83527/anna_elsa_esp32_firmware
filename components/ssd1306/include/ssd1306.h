...
public:
  ...
  0x40 : Data Stream
   */
  static constexpr uint8_t OLED_CONTROL_BYTE_CMD_SINGLE = 0x80;
  static constexpr uint8_t OLED_CONTROL_BYTE_CMD_STREAM = 0x00;
  static constexpr uint8_t OLED_CONTROL_BYTE_DATA_SINGLE = 0xC0;
  static constexpr uint8_t OLED_CONTROL_BYTE_DATA_STREAM = 0x40;

  // Fundamental commands (pg.28)
  static constexpr uint8_t OLED_CMD_SET_CONTRAST = 0x81; // follow with 0x7F
  static constexpr uint8_t OLED_CMD_DISPLAY_RAM = 0xA4;
  static constexpr uint8_t OLED_CMD_DISPLAY_ALLON = 0xA5;
  static constexpr uint8_t OLED_CMD_DISPLAY_NORMAL = 0xA6;
  static constexpr uint8_t OLED_CMD_DISPLAY_INVERTED = 0xA7;
  static constexpr uint8_t OLED_CMD_DISPLAY_OFF = 0xAE;
  static constexpr uint8_t OLED_CMD_DISPLAY_ON = 0xAF;

  // Addressing Command Table (pg.30)
  static constexpr uint8_t OLED_CMD_SET_MEMORY_ADDR_MODE = 0x20;
  static constexpr uint8_t OLED_CMD_SET_HORI_ADDR_MODE =
      0x00; // Horizontal Addressing Mode
  static constexpr uint8_t OLED_CMD_SET_VERT_ADDR_MODE =
      0x01; // Vertical Addressing Mode
  static constexpr uint8_t OLED_CMD_SET_PAGE_ADDR_MODE =
      0x02; // Page Addressing Mode
  static constexpr uint8_t OLED_CMD_SET_COLUMN_RANGE =
      0x21; // can be used only in HORZ/VERT mode - follow with 0x00 and 0x7F =
            // COL127
  static constexpr uint8_t OLED_CMD_SET_PAGE_RANGE =
      0x22; // can be used only in HORZ/VERT mode - follow with 0x00 and 0x07 =
            // PAGE7

  // Hardware Config (pg.31)
  static constexpr uint8_t OLED_CMD_SET_DISPLAY_START_LINE = 0x40;
  static constexpr uint8_t OLED_CMD_SET_SEGMENT_REMAP_0 = 0xA0;
  static constexpr uint8_t OLED_CMD_SET_SEGMENT_REMAP_1 = 0xA1;
  static constexpr uint8_t OLED_CMD_SET_MUX_RATIO =
      0xA8; // follow with 0x3F = 64 MUX
  static constexpr uint8_t OLED_CMD_SET_COM_SCAN_MODE = 0xC8;
  static constexpr uint8_t OLED_CMD_SET_DISPLAY_OFFSET =
      0xD3;                                                 // follow with 0x00
  static constexpr uint8_t OLED_CMD_SET_COM_PIN_MAP = 0xDA; // follow with 0x12
  static constexpr uint8_t OLED_CMD_NOP = 0xE3;             // NOP

  // Timing and Driving Scheme (pg.32)
  static constexpr uint8_t OLED_CMD_SET_DISPLAY_CLK_DIV =
      0xD5;                                               // follow with 0x80
  static constexpr uint8_t OLED_CMD_SET_PRECHARGE = 0xD9; // follow with 0xF1
  static constexpr uint8_t OLED_CMD_SET_VCOMH_DESELCT =
      0xDB; // follow with 0x30

  // Charge Pump (pg.62)
  static constexpr uint8_t OLED_CMD_SET_CHARGE_PUMP = 0x8D; // follow with 0x14

  // Scrolling Command
  static constexpr uint8_t OLED_CMD_HORIZONTAL_RIGHT = 0x26;
  static constexpr uint8_t OLED_CMD_HORIZONTAL_LEFT = 0x27;
  static constexpr uint8_t OLED_CMD_CONTINUOUS_SCROLL = 0x29;
  static constexpr uint8_t OLED_CMD_DEACTIVE_SCROLL = 0x2E;
  static constexpr uint8_t OLED_CMD_ACTIVE_SCROLL = 0x2F;
  static constexpr uint8_t OLED_CMD_VERTICAL = 0xA3;
...
private:
       ...
       Conversion is done via following procedure:
            ...
            for (int code = 0; code < 128; code++) {
                    ...
                    for (int w = 0; w < 8; w++) {
                            trans[w] = 0x00;
                            for (int b = 0; b < 8; b++) {
                                    trans[w] |= ((font8x8_basic[code][b] & (1 <<
     w)) >> w) << b;
                            }
                    }
  ...
  static constexpr uint8_t font8x8_basic_tr[128][8] = {
